...
"""
LTX-2 sink-bidirectional refiner.

Wraps ``diffusers LTX2VideoTransformer3DModel`` + ``LTX2TextConnectors`` to
provide a clean interface for cross-model refiner reuse.

The refiner checkpoint is a standard LTX-2 transformer plus text connectors.
Diffusers already owns those modules, but its public transformer forward always
runs the audio stream and does not expose the streaming sink/current video
self-attention mask that this refiner was trained with.

This wrapper keeps the custom surface narrow: load diffusers components,
encode the prompt through Gemma + ``LTX2TextConnectors``, and run a
video-only forward through the diffusers transformer blocks. The only local
attention code is the streaming sink/current split, implemented with diffusers
attention modules without materializing the full sequence-by-sequence mask.
"""
...
class DiffusersLTX2Refiner(nn.Module):
    """Small adapter around diffusers LTX-2 transformer + connectors for video-only refinement."""
    ...
    def refine_latents(
        ...
        *,
        ...
        if progress:
            ...
            iterator = tqdm(iterator, desc="[refiner] LTX-2", unit="step")
...
def _streaming_self_attention(
    *,
    ...
    """Run LTX-2 self-attention with the sink/current streaming mask.
